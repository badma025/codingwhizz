...
const Blog: NextPage<IProps> = ({ posts }) => {
  ...
  return (
    <div>
      <main className="font-Poppins  mx-auto min-h-screen max-w-[1500px]">
        <Header />
        <Search />

        <section className="mx-auto mt-20 max-w-7xl">
          <div className="mx-auto flex flex-col items-center justify-center space-y-1 text-lg sm:text-xl font-semibold tracking-wide xl:-mb-16">
          ...
          <section className=" mx-6 grid  mt-10 grid-cols-1 items-center justify-center gap-3 p-2 sm:grid-cols-2 md:mx-auto md:gap-6 md:p-6 lg:grid-cols-3">
            {posts.map((post: Post) => {
              return (
                <div
                  className="group
             w-full cursor-pointer space-y-3 overflow-hidden    rounded-lg  border-2 border-gray-500 p-6 xl:mt-24"
                >
                  <Link
                    href={`https://codingwhizz.org/blog/article/${post.slug.current}`}
                  >
                    <img
